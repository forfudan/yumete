...
| 302 | **命令行該不該自己佔一行** | tui | P4 | 常駐在狀態列下面，命令、搜索、訊息、常駐鍵都走它 [^302] | Fixed 2026-09-13 |
...
[^299]: 2026-09-08：「目前底部我们有一行状态栏，上面还有一个信息栏（提示栏）。
    ...
    **第二步**：命令行照 helix 的形狀搬到**狀態列下面**、常駐一行（#302 的取法），
    ...
    **這樣算不算白拿一行？不算，也不是為了那一行。** 命令行常駐，等於一行換一行。
    真正拿回來的是四樣：① 訊息一眼看得出不是稿子（它在狀態列**下面**，那一帶不是正文
    的地盤）；② 每一格有自己的職責——狀態列答「我在哪」，命令行答「我剛打了什麼、系統
    回了什麼」，面板答「這件事要你讀一段或選一個」；③ 搜索與命令不再搶狀態列，打字的
    時候「我在哪」不消失；④ 四種面板一套代碼。

    **落地（2026-09-13）。** 四件事一批做完：
    ① `page_areas`（`lib.rs`）先扣命令行再扣狀態列，於是**命令行在狀態列下面**、常駐一行，
    開關從 `[editor] hints` 改名 `[editor] command_line`（關掉則三樣東西退回狀態列輪流用）；
    ② `:` 命令、`/` 搜索、以及一行寫得完的訊息都畫進那一行，狀態列打字時不再消失；
...
[^302]: 2026-09-08 看 helix 順帶記下的兩條，都關於底下那幾行：「helix 的底層狀態欄
    ...
    **2026-09-13 定了：常駐，不是臨時借。** 照 helix 原樣——狀態列下面一行，一直在，
    空着的時候就空着。所以上面那個「借的那一行從哪裏來」的問號不存在了：頁面高度從一開始
    就少那一行，正文永遠不抖。**空行不是浪費**，那是這一格的職責：狀態列答「我在哪」，
    命令行答「我剛打了什麼、系統回了什麼」，一格一件事，空着正說明現在沒有那件事。它用
    正文的底色而落在狀態列**下面**，所以既不與正文混淆，也不必再畫一道框。
...
[^387]: 2026-09-11 的六路審閱，輸入法那一路用真 pty 測出來的（`--shot` 測不到，見 #385）。
    ...
    **`frame.area()`，整個窗口**，不是正文區。於是「貼着窗口底」是合法的，而狀態列就在
    ...
    狀態列不行，它答的是「我在哪」。
    ⚠️ **2026-09-13 起這句作廢**：提示行沒有了（#299），命令行接了它的活並排在狀態列
...
[^399]: 2026-09-11 的六路審閱，上手那一路報的。表格裏按 `Tab` 確實會在「按字」與「按格」
    ...
    **落地（2026-09-12）。成因不是「不說」，是說錯了鍵。** 狀態列右邊一直寫着
