...
[^293]: 2026-09-08 的設計：「文檔可以有兩個側欄，左側欄是文件、目錄等信息（不需
    ...
    **字典是右欄裏一格「跟着光標走」的（2026-09-13 定，改寫上面那條）。** 它不是
    一個 `Tab` 輪得到的常駐視圖：查一個字才出現，光標一離開那個字**立刻收**——上一
    刻畫的是表格詳情就退回表格詳情，上一刻右欄根本沒開就把右欄關掉。所以「輪到空
    的畫『這裏沒有』」那條對字典不適用（它從不空着待在那兒），對表格詳情、百科那
    幾格照舊。⚠️ 這使右欄的**內容有主人**：光標決定畫哪一格，`Tab` 只在**常駐的那
    幾格**之間走。
    ...
    **做的順序**（2026-09-13 定）：
    ...
    | 三 | 字典改成跟光標走；表格詳情從 `t i` 那條右豎條搬成正經面板 | 兩側同時有東西，槽位真的解耦了 |
    | 四 | 哪個面板在哪一側交給配置 | |
...
[^418]: 2026-09-12：「markdown 自動補全，没有補全有些痛苦。比如列序號、自動章節
    ...
    **三 · `[[` 的索引掃哪裏（2026-09-13 定）：當前檔案所在的那個目錄，連同它的子目錄。**
