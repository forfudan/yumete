...
- **`:search` 與 `:replace` 也要模糊匹配（fzf 那一路），做成一個開關。** 挑選器
  ...
  正則對這件事幫不上忙。
  ⚠️ 要想清楚的三件：① **開關擺在哪**——搜索面板上的第四個勾，還是 `[editor]` 一行，
  還是 `:search` 的一個參數；② **模糊與智能大小寫、整詞、正則三個現有開關怎麽疊**
  （模糊開着的時候「正則」那一格應當變灰，兩者不是一回事）；③ **CJK 的子序列匹配要按
  字算**，不是按字節，而且「連續」這件事在漢字上比在拉丁字母上更重要——
  挑選器那份打分能直接借過來的是哪幾條，要量過再說。
  **medium**
