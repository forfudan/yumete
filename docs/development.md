...
| 388 | **`-t` 遇到撕裂的表一聲不吭** | cli | P1 | 同一個判斷，`t t` 有話說 [^388] | Open |
| 389 | **`--shot` 尺寸認不得就靜默退回 100×30** | cli | P1 | 一個兜底廢掉了六路審閱的證據 [^389] | Open |
| 390 | **大綱把目錄最後一條當成正文第一章** | core | P2 | 目錄與正文之間夾着版本說明就中 [^390] | Open |
| 391 | **一處引號沒關，別處合規的欄跟着被拆** | core | P2 | 只壞顯示不壞位元組，但沒有提示 [^391] | Open |
| 392 | **基本表格丟掉它本來知道的欄名** | core | P2 | 回落成 1／2／3 [^392] | Open |
| 393 | **文檔說 41 個命令，面板數出 47 個** | docs | P2 | 摺完之後沒有人再數一遍 [^393] | Open |
| 394 | **極窄終端的狀態列硬截斷到半個字** | tui | P2 | 該先丟次要欄位，不是攔腰切 [^394] | Open |
| 395 | **孤立的 `\r` 被當成換行數進去** | core | P2 | 行號與 `wc -l` 差一行 [^395] | Open |
...
[^387]: 2026-09-11 的六路審閱，輸入法那一路用真 pty 測出來的（`--shot` 測不到，見 #385）。
    ...
    邊框打亂。tmux 裏開一個小格子是常事，不算冷門。修法是面板高度按可用行數 clamp，
    不夠就降級（少畫幾條候選，或者改成單行）。**small**
...
[^388]: 2026-09-11 的六路審閱，寫文檔那一路報的。同一份撕裂的 csv：
    ...
    `-t` 卻沒進表格模式，只會懷疑自己的檔案。修法：`-t` 判定失敗時把 `t t` 那句話說出來。
    **small**
...
[^393]: 2026-09-11 的六路審閱，上手那一路數出來的。`docs/manual.md:897-898` 與
    ...
    ⚠️ 這是 #300 第四條的原樣重演：**文檔裏的數字過期了不會報錯。** 修法是數一遍改對，
    順帶想想這個數字值不值得寫死在文檔裏。**small**
...
[^395]: 2026-09-11 的六路審閱，邊界那一路報的。一份主要用 `\n` 收行、中間夾了一個孤立
    ...
    已驗證**不影響存盤位元組**（`:w` 之後 `cmp` 逐位元組相同），純粹是行號與實際脫節。
    **small**
...
[^400]: 2026-09-11 定的：中文名從「宇浩終端文字編輯器」改成 **「宇夢終端編輯器」**，
    簡稱「宇夢編輯器」。理由是**名字要自己解釋自己**：`yumete` 四個字母擺在那裏，
    `yume` ＝ 宇夢，中文讀者一眼對得上；而「宇浩」和 `yumete` 之間沒有可見的聯繫，
    得有人告訴你那是 `Yuhao IME text editor` 的縮寫才懂。何況**嵌進來的確實是宇夢
    引擎**——宇浩是方案家族（光華、星陳、日月、冰雪），一個編輯器不是一個方案。
...
[^401]: 2026-09-11 定的：像 helix 的 `hx` 一樣給一個簡稱 **`ye`**。
    構法是一樣的——`hx` ＝ **h**eli**x** 的首字母加尾字母，`yumete` 照這個構法正好是
    **y**umet**e**。（`yt` 在很多人手裏會讀成 YouTube；`ym` 手感不好。）
