...
**段落之間不空縱，改成首行縮進兩格（作者同日）**。空一縱在一頁紙上是一整條白，在只有十來
縱的小框裏更是把兩段推得老遠。做法是在段首寫兩個全角空格再交給折行器——它就當是兩個字，
第二縱起自然頂格；空段落直接丟掉。標題那一縱不縮進，頂着框走。📌 作者同日提過另一種
分法——「標題縮進兩格，段落縮進一格」——當場又說「先這樣不變」。要改就是改那兩個全角空格
的個數，一行的事。
