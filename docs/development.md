...
PICK 面板在窗口高 ≤14 時畫穿狀態欄；橫排注音不撐開基字（竪排撐了，手冊說該撐）；表格列寬
按**可見行**重算、滾一下整張表左右跳；搜索面板的 `Tab` 在框裏無效、在框外是換視圖（而面板
頁腳與手冊都寫「下一格」）；`空格 t e` 開出來的 `t.toml` 被當成表格畫；邊欄固定 23 欄，
30 欄的窗口裏正文只剩 2 欄字；大綱／文件樹的截斷沒有省略號。
