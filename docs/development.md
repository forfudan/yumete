...
[^293]: 2026-09-08 的設計：「文檔可以有兩個側欄，左側欄是文件、目錄等信息（不需
    ...
    * **高度**：下層要多高給多高，最多占這一槽的一半；上層拿剩下的。上層空着就整格
      都給下層。
    * **下層只讀，但滾得動**（字典二十來行，陸臺港三家拆法更長；一行二十八欄的詳情
      同理）。滾就要拿鍵，所以下層也是 `C-w` 環上的一站：`j`／`k` 滾，`h`／`l`／
      `Enter` 什麼都不做。⚠️ 這一點自洽：鍵在面板裏的時候正文光標不動，那個字還成立，
    ...
    **做的順序**（2026-09-13 定）：
    ...
    | 三 | 每槽分兩層；詳情搬進右槽下層，字典變成臨時層 | 兩側同時有東西，槽位真的解耦了 |
    | 四 | 哪個面板在哪一側交給配置 | |
