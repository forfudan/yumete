...
| where | what |
| --- | --- |
| `composes` | `Mode::Picker` added — the gate that lights the preedit, the panel, lone-Shift and `Shift+Space` together |
...
1. **`r` stays where Helix has it, and learns to type Chinese.** Replacing a
   ...
   `composes_here(editor)` — the lone-Shift toggle (`tui/lib.rs:492`), `Shift+Space`
