...
⚠️ 順帶一個更隱的：**折行器的預算是格，一縱的長度是字**。半角字一格一個，所以 52 格裝得下
28 個字，而 `tall` 是 26——詞條裏有「500」「>」這種就會多出兩行。折完再按字數硬切一次。

**段落之間什麽都不加，最緊（作者同日：「縮進不好看，不要縮進了，也不需要空行，就用最
compact 的狀態」）**。兩種分段法都做出來看過：空一縱在只有十來縱的小框裏把兩段推得老遠，
首行縮進兩格看着像斷了。空段落直接丟掉，段與段之間就是相鄰的兩縱——一縱的邊界本身已經是
分隔。
