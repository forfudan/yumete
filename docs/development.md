...
- **一格的深度是固定的**，短的補空、長的截斷畫「…」（作者：「超過就截斷」）——格子對齊本身就是
  那張網，一格自己拉深，網就碎了。要看全文按 `t t`。
...
📌 **還差一件：帶還没對齊。** 格子的補空是 `mdtable` 按**格**算的（一個漢字兩格），而一縱是按
**字**算的——六格的列裝三個漢字是三槽，裝兩個漢字加兩個空格是四槽，於是各行的帶錯開。要治就
得讓竪排那一路按字數補空（`table_padding_on_line` → `mdtable::widths`／`pad` 換一個度量，
連 `pad_cache` 的鍵一起），而**不能動 `mdtable` 給文件排版的那一路**——那會改到文件本身的字節。
