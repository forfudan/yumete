...
**格子兩邊的空格在竪排下不上頁面**（作者 2026-09-19：「表格的單元格寬度有問題，不夠
compact」）。一格寫作 `| 級別 |`，那兩個空格橫排值得留——它們佔一個漢字的寬，把字和牆隔開；
立起來之後它們一個佔**一整行**，四列的表白花八行。`Editor::table_cushions_off_the_page` 把
它們藏掉（`|` 留着，帶就是從它畫出來的），只在竪排，只在表格行。順帶治好了「標題欄的分割綫
沒有對齊」：`| --- |` 裏那兩個空格從前把那道牆打成了虛綫。
