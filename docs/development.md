...
**四檔四個形狀，不是四個顏色**：`•` 錯、`!` 警告、`i` 說明、`·` 建議。`theme.rs` 的
`word_hue` 和剪口那一格爲同一件事寫過同一條理由——百個男人裏有八個分不出紅綠，而這一格
只有一格寬，單靠顏色說話等於對他們什麼都没說。

⚠️ **這四個字是量過的，不是挑好看的**（2026-09-21）。第一版是 `●▲◆·`，畫出來一眼就是
「紅圈圈是個半圓？」。在 CJK 等寬字體（LXGW 文楷 Mono GB）裏量：**一格 7.5px，而
`●`／`▲`／`◆` 都是 15.00px——整整兩格**，`•`／`·`／`!`／`i` 纔是 7.50px。兩格的字擠進
一格，終端不是裁掉半邊就是把整行推開一格，而這一欄存在的全部理由就是**不動版心**。
所以形狀讓位給寬度，並釘了一條測試：**方塊區（U+2580–U+25FF）的字一個都不許進這一格**。
...
**假服務器測不出這個**：`spawn()` 成功了，`try_wait` 立刻說它没了，而我的代碼把這讀成
