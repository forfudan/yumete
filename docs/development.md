...
| 304 | **`e` 把上一個詞的尾巴和標點一起圈進來** | core | P2 | 錨點留在原地；`w` 是對的，`e`／`b`／`E` 不是 [^304] | Proposed |
...
[^304]: 2026-09-08 報的：「中文的標點似乎沒有算成標點，導致 `w` 和 `e` 會把標點含進去。
    ...
    **現象屬實，成因不是標點的分類。** 標點分得好好的：`category`（`yumete-cjk/src/word.rs:46`）
    把 `，` U+FF0C 與 `。` U+3002 都判成 `Punctuation`，`is_cjk` 也不收它們，所以
    `word_ranges` 與詞典分詞器都把一串標點切成自己一段。真正錯的是**錨點**。
    拿「師父，我把他帶來了」逐鍵看（`--shot --html`，選區底色 `#5F5F60`）：
    `e` → 「師 · `父，`」 → 「`，我`」——每一次都是**上一個詞的最後一字 ＋ 這一個詞**，
    夾在中間的標點於是兩頭都跟着走。
    **`w` 是對的**：`select_word_forward`（`editor/matching.rs:325`）自己算錨點——
    取不動就改取下一個詞，`anchor = next`——所以一次選區要麼是一個詞，要麼是一段標點，
    不會混。**`e`／`b`／`E`／`B` 走的是另一條路**（`editor/keys.rs:647` 一帶）：
    算出一個位置就 `select_to(p)`，而 `select_to` **把錨點留在光標原處**。
    修法：給 `e` 一個和 `w` 同形的 `select_word_end`——錨點落在**它要去的那個詞的開頭**，
    `b` 照鏡子做一份。四個鍵一起改，別只修 `e`。
    ⚠️ **另一半要先定**：`w`／`e` 到底該不該**停在標點上**？現在會停（`w` 從「父」按一下，
    選區就是那個「，」），helix 也停——標點在它那裏是自成一類的詞。報這一條的意思更像
    「一個都別停，直接跳到下一個真詞」。兩種都說得通，但那是**設計決定**，不是這個 bug，
    決定之前先把錨點修對。
    ⚠️ **量它的時候**：`--shot` 沒有終端機的光標，而分詞疊色（一深一淺）畫在選區上面，
    所以**只有一個字的選區在圖上看不見**——別據此以為 `w` 沒選中東西，看狀態列的
    `Col` 與底下那個 U+ 碼位。**small**（錨點）＋ **一個設計決定**（要不要停在標點上）
