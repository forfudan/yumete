...
作者看着一屏上擠着百科詞條和 `:` 選單說的：「一次只會出現一個面板，那麽輸入命令的
時候百科窗口自然就會消失」。⚠️ 原先是四個無條件的呼叫，後畫的蓋住先畫的——而那裏
的註釋寫的正好相反（說「半按的鍵序列贏」，代碼卻是便簽後畫、蓋在鍵表上）。

**浮框的底色離紙五檔（第 85 檔，作者 2026-09-18 定）**。從前浮框畫在紙自己的顏色
上，只靠一條綫說它從哪裏開始：「我感覺他和正文還是有些混在一起」。先試過**雙層圓角
外框**——橫竪各多吃四格，四分之一屏本來就小，挑選器兩欄各套兩層反而更亂；作者看過
圖後定：「底色下降一些或者上升一些，從而和紙色分開，這個比雙層框綫更好」。一格不多佔，
而且它是**檔號**不是顏色，淺色主題下自己反過來。

⚠️ **面板裏的文字不設底色**（作者同日：「命令行文字嚴格意義上來說底色是透明的，下面
...
**浮框多大：橫排寬 ≤ 2/3、高 ≤ 1/3；竪排轉置（作者 2026-09-18 定）**。舊規矩是「長寬
各一半」，理由是「哪個角都躲得開光標」。作者指出**護住光標的是高度那一半**——框占上三分之一
...
**百科浮框底下不再寫檔名**（作者同日：「我不覺得百科面板的下方有任何必要顯示百科詞條所在的
文件名」）——`gd` 本來就過得去，那一行還占掉一行詞條。

**竪排頁面下，百科的正文也竪排（作者 2026-09-18 定）**。`Panel` 多一個
...
**標題自己是最右那一縱，金墨；框上不再寫名字（作者 2026-09-18 定，三個辦法裏的丙）**。
竪排的書就是這麽做的。另外兩個辦法是「標題壓着外框竪排」和「照舊寫在框的左上角」，都被
否了。金與墨的分別已經夠把標題和正文分開，所以**標題和正文之間不空一縱**（作者：「不需要，
因為有顏色的區別」）。

⚠️ **竪書的截斷要自己寫，橫排那一段不能借**（2026-09-18 作者報「高度不對，而且文字被截掉
...
更糟——那一刀不認禁則，「，」被切到了下一縱的頭上（作者當場看出來了）。`zong` 那一支本來就是
按字算的、禁則在裏面，頁面上的竪排走的就是它。

**「…」接在末縱的腳下，不自己占一縱**（作者同日：「最後的省略號後面有個空行」）。自己占一縱
的話那一縱只有一個字、底下一大片白，讀起來是「這裏空了一行」，而它要說的是「話還没完」。

**表格在浮框裏畫成表格，永不折行（作者 2026-09-19：「橫排的浮窗也渲染一下吧，然後讓他不要
...
**竪排的表格順時針轉九十度，只讀（作者 2026-09-18 定，先做浮框這一半）**。竪排讀下、縱往左，
...
- **每條帶按需分深**（作者 2026-09-19：「空間夠就渲染全部，不夠再摺疊，儘量保證撐滿整個
...
**頁面那一半（2026-09-19 起了頭）**。作者指出一件我說錯了的事：**轉九十度本來就是一一對應
...
**格子自帶的空白在竪排下不上頁面**（作者 2026-09-19：「表格的單元格寬度有問題，不夠
...
別的帶旁邊。這只在對齊過的文件上看得見，手打的表看着好好的——作者那張圖就是這麽來的。
...
⚠️ **「一屏幾行」竪排要問別的數**（作者 2026-09-19：「整個表格都在可視範圍內哦」）。表格只
...
**牆站在自己那一縱的頭上，交叉畫 `┼`**（作者 2026-09-19：「竪綫怎麽不是在那一縱正中？爲什麽
...
⚠️ **虛字要接下面那一層的底色，不能自己刷頁面色**（作者同日：「表格隔行的底色沒有正確
...
的表格按設計是**只讀**的（作者：「進入表格後還是只讀狀態，必須 tt 才能編輯」）。所以補空問
`table_padding_on`，開門問 `table_view_opens_itself`（多一句 `layout == Horizontal`）。


**三色一套規矩，橫竪通用（作者 2026-09-18 定）**：名字是金，**章節行是灰**，正文是墨。章節行
...
一類）。作者 2026-09-20 定：**不改標記的規矩，把報告做誠實**——誤標一個「小明」在「他小明
...
**繁體正本用大陸詞彙，生成器只換字形**（作者 2026-09-19 定）。簡體版是生成物，而
...
- **行號旁邊那一格用底色顯示 `git diff`**（作者 2026-09-19：「非常重要」，P1）。helix 有
...
- ~~**`:search` 與 `:replace` 也要模糊匹配。**~~ 做完了（2026-09-20），三個岔路由作者當場
  定下，記在這裏免得重開：
...
作者 2026-09-20 定下的重構：**一臺機器，兩種語法**。翻譯表（vim 預設把 `x` 播成 `;D`）
...
做法就是那句話：**一個動作，兩種讀法**（作者 2026-09-20「vim 的 `w` 獨立的時候是跳轉，
...
走完停在空格上」（翻譯表的産物）；另一條的註釋寫着作者 2026-09-18 的原話——
**「vim w 是跳到詞頭，這個我們肯定没辦法實現」**——B3 實現了它。改動的原委寫進了那兩處註釋。

⚠️ 我自己弄丟過一個：`j`／`k` 作爲行式動作（`dj`）第一版漏在表外，舊測試當場抓住。
...
行號後面本來就有兩格空氣（`GUTTER_AIR`），貼着正文的末一格 2026-09-19 歸了 git 改動條
（#55／#298）。**診斷落在它前面那一格。** 於是兩件事都落地了，版心還是一欄都不用挪——
「一個功能上線那天頁面重排」是這個設計從第一天起要躲開的事。

⚠️ **helix 把診斷畫在行號左邊，那要多一欄。** 這裏放右邊，是因爲左邊那一欄得從正文身上
要，而那一欄正文在中文裏是**一個整字**。

**四檔四個形狀，不是四個顏色**：`●` 錯、`▲` 警告、`◆` 說明、`·` 建議。`theme.rs` 的
`word_hue` 和剪口那一格爲同一件事寫過同一條理由——百個男人裏有八個分不出紅綠，而這一格
只有一格寬，單靠顏色說話等於對他們什麼都没說。⚠️ 四個字符**全是 East Asian Ambiguous**，
CJK 字體的終端上佔兩格，多出來的那一格會把整行推歪；所以照剪口那一套量一量再畫，量出
兩格就退回 `x` `!` `i` `.`。
...
- **L2**：JSON-RPC 的分幀（`Content-Length` 那一套）、一條客戶端線程餵現成的 `mpsc`、
  `initialize`／`didOpen`／`publishDiagnostics` 接進事件循環。**不要 async、不要 tokio、
  不要 `tower-lsp`**，新依賴只有 `serde_json` 一個。
- **L3**：`didChange` 先走全量同步、服務器崩了要能自己重起而不帶走編輯器、
  `[lsp.rust] command = "rust-analyzer"` 那樣的配置（形狀照 helix），**只在真的打開了
  那種檔的時候纔起**。
...
[^419]: 2026-09-13：「grep replace 我不喜歡」，改照 VSCode 邊欄搜索那一扇。決定逐條
    ...
    * ⚠️ **`Tab` 從找什麼落到換成什麼，鍵要留在打字裏。** 兩個框上下挨着、一前一後填，
    ...
    24 行的窗口下只裝得下 **6 欄 × 9 行 ＝ 54 格**，而別名在那張表裏**各佔一行**
