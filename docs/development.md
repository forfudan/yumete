...
| 375 | **狀態行明明放得下，卻折成兩行** | tui | P3 | 右邊那截的寬度算少了 [^375] | Open |
| 376 | **表格裏 Insert 的方向鍵走不出這一格** | core | P3 | 讓上下左右跨格、行末跨到下一行 [^376] | Open |
| 377 | **狀態行一折，整頁就糊了** | tui | P1 | 幀比終端高一行，終端一捲，滿屏重影 [^377] | Open |
| 378 | **csv／tsv 在 tb／tf 下不排齊** | core | P2 | 分隔符是個值，四種標點走同一條路 [^378] | Fixed 2026-09-11 |
| 379 | **正文頁的表格沒有欄名可看** | tui | P2 | tb 也給欄號；滾到表格中部時把欄號與欄名釘在頂上 [^379] | Open |
...
[^366]: #315 把**動不了的**那些代價拿掉了（注音、hash），剩下的是真的：往一段裏打一個字，
    那一段確實得重折。可重折是從段首開始的，於是一段一百萬字的純文字（匯出、日誌、貼進來的
...
[^375]: 同一張截圖：狀態行右邊的 `U+0009 · Basic Latin` 被折到了第二行，只掉下來一個
    `n`——而那一行明明還有空位。`lib.rs:5290` 一帶用 `str_width` 量左半截、算出 `room`，
    再挑一個放得下的右半截。少算了一格，就多出一格、折行。

    嫌疑：`·`（U+00B7）是 East-Asian **Ambiguous**，而 `str_width` 跟着終端的
    `ambiguous_width` 走；若編輯器按窄的算、終端按寬的畫，就正好差一格。這與 #327 是
    同一族的問題——那一條分開了「存檔的寬度」與「畫面的寬度」，這一條要問的是畫面這
    一半有沒有問對終端。量之前先復現，別照猜的修。**small**
...
[^377]: 作者，2026-09-11：「这个码表文件，tt 进入表格视图，我的光标在两列间移动的时候
    ...
    幾乎可以肯定和 #375 是同一件事的兩張臉，作者也是這麼猜的。#375 是狀態行明明放得下
    卻折成兩行；一折，這一幀就比終端**高了一行**，終端只好往上捲，於是上一幀畫過的東西
    全部留在原地——就是這張截圖。

    而「在兩列間移動」正好解釋了觸發時機：狀態行右半截寫的是光標底下那個字元，光標
    落在 TAB 上時它是最長的那一串（`CJK Uni U+0009 · Basic Latin`），也就正好是撐破
    那一行的時候。

    所以先量 #375 那一格差在哪（`·` 是 Ambiguous 的嫌疑仍在），修好它多半這一條也就沒了；
    但**折行本身不該把幀撐高**——狀態行放不下時該截斷，而不是換行，這是第二道閂，值得
    分開做。復現：`yumete mabiao-ling.txt`，`tt`，光標在編碼欄與字欄之間左右走。**small**
...
[^379]: 作者，2026-09-11：「tb 模式可不可以也标注列号（和 tf 模式一样）。然后 tb 和 tf 在
    ...
    **兩件事，難度差得遠，分開做。**
    ...
    一、tb 也給欄號條。合 tb 自己那條法——作者，2026-09-11：「tb 的原则是只能多字
    （标注）不能少字。」欄號條是加在表格上頭的一行標註，不藏、不摺、不替換任何字，
    正是「多字」。tf 已經在畫，把它從 全 的條件裏挪出來即可。**small**

    二、滾到中部時把欄號與欄名釘在頂上。這是窗格裏的 grid 從第一天就有的東西，它自己
    的模組開頭把理由寫死了：「the frozen header so a column can be named, the frozen
    row numbers so a row can be」。難的是 grid 擁有自己那一格畫布，而正文頁沒有——釘一
    行在頂上，意味着在正文的行上蓋一行**不屬於那個位置**的字，折行、光標、點擊圖三者
    都得知道那一行不是文本。#212 的補白是「畫出文件沒有的字」，這是「在文件的字**上面**
    畫」，不是同一道門。

    欄名從哪來也得想清楚：Markdown 是表頭那一行；delimited 檔案是 `.yumete/tables/` 裏
    的 schema，沒有 schema 就用檔案自己的首行。碼表兩樣都沒有，那就只剩欄號——也就是說
    這個功能對它最要緊的那個檔案，只能給出一半。**medium**
