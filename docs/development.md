...
⚠️ **還欠一個逃生口**：想主動斷一段的時候没辦法。helix 用 `C-s`，而**終端裏 `C-s` 是
XOFF**（會凍住屏幕，除非 `stty -ixon`），所以那個拼法這裏不能照抄；vim 的正統拼法是
`C-g u`（`:h i_CTRL-G_u`，字面意思就是「斷開 undo 序列」）。記着，等定。
...
⚠️ **`空格 K`（側欄版 hover）留着不占**：它要搭的就是這一套機制，等這一套落地了再接，免得做出
兩套行為不一樣的面板。
