...
⚠️ **只認全拼**，同高級搜索——作者定：「不一下子提供太多东西直到真有人要」。
...
⚠️ **往紙色靠，不往梯子中央靠。** 兩種都畫出來比過（作者提的是中央那一檔）：中央那一檔把
**底色也拉亮了**，那一片反倒比原先顯眼，回過頭來跟浮着的那扇窗搶注意力。作者看完自己的話：
「靠纸色的话后方背景不会变，只是墨色变淡」。⚠️ **亮色主題下這條要重量一次**，那裏「紙」是
白的。

## 5.12.44 一個名詞：區域（2026-09-26）

作者定：「可不可以把工作区和侧边栏统一成一个概念「区域」以简化思维模型。现在我们有了
space 1 2 3 4 这样的快捷键，我们其实就不大需要太多快捷键了。」

**病在哪**：`空格` 菜單裏躺着**兩套詞彙說同一件事**。
...
作者要一張「各種情況下邊欄多寬」的表，量出來之後發現那張表沒法畫——規矩有六套。
...
⚠️ **檔位名就寫分數，而且分母都是十**（作者定）。「綽」和「寬」是近義詞，狀態欄寫「邊欄：
...
`w` 照走檔位（作者定：「照走，状态栏说档位名」），窗口一拉大它就真的寬了。

## 5.12.46 大綱兩件（2026-09-26）

### 一、`#128` 被當成一級標題

作者報的：「比如 Issue #128，如果 #128 手动换行了，那么 #128 会被认定为一个一级标题。这个
...
**縮到第三級封頂**（作者定）。四級起與三級對齊——`5.8.11` 一看就比 `5.8` 深一層，層級不必
用欄位再說一遍。
