...
| 421 | **設置要有一扇面板，別讓人對着 toml 發呆** | config+tui | P1 | 版面那一組 12 項落地；其餘七組在 `LATER` 裏排隊（§5.12.24） | Fixed 2026-09-24 |
...
「版面」那一組是滿的（12 項），其餘七組在 `LATER` 裏掛着（約 75 個鍵，含 24 個色位）。
那張一致性測試讀的是三張表的並集，所以漏一項照樣紅。

⚠️ **`main.rs::press` 那一份鍵路由和主循環是兩份**，還沒合。後果有兩層：面板最重要的安全
行為（有改動沒存不許一下走）在 `--shot` 下拍不到；而 `--keys` 裏帶 `:w` 會真的改寫使用者
的 `~/.config/yumete/config.toml`，沒有任何提示。

⚠️ **面板開着期間那個檔被外面改了，`:w` 整份蓋掉**：`save()` 拿的是開面板那一刻的
`sheet.text`，沒有 mtime／內容比對。
