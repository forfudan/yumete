...
要是這中間終端崩了、電斷了，下次打開同一個文件，狀態欄會說有一份更新的搶救稿。它
**不會**自動載入——悄悄給你看一份和硬盤上不一樣的文字，是讓人分不清自己在讀哪一版
的開始。`:recover` 載入（載入以後 `u` 可以退回硬盤上那版，`:w` 才算採納），
`:recover!` 直接丟掉。

沒有名字的臨時緩衝區沒地方放副本，所以沒有。`[editor] autosave = false` 可以整個
關掉。
