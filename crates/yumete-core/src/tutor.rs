...
`C-Space`（Control 加空格）開輸入法，再按一次關掉。
開着的時候，單獨輕按一下 **Shift** 在中／英之間切。

  ⇒ 按 `i` 進 INSERT，按 `C-Space`，在下面這一行後面打幾個字。

    我要在這裏寫一句話：

打不出來？狀態列會說輸入法有沒有裝好；`:yume` 說得更詳細。
`:scheme` 換方案（靈明、星陳、卿雲、日月、拼音）。
...
`:zong 24` 定一縱多少字，`:bands 2` 把一頁分成上下兩段。
`:hanging on` 讓句讀掛到字旁邊去，像排印的書。
再按一次 `:layout` 回橫排。

  ⇒ 按 `:indent 2`，Enter。段首空了兩格，段間那些空行收起來了——
     檔案沒有變，變的是版面。`:indent 0` 回去。
...
`h` `j` `k` `l` 按格走，`Tab` 換成按字走。
`i` 進格子打字，`c` 換掉整格，`t o` 加一行，`t d` 刪一行。

  ⇒ 光標放到「相」那一行的拆分格上，按 `Tab` 換成按字，走到「木」上按 `gd`：
     跳到「木」自己那一行。（按格的時候問的是整格，沒有哪一行叫「⿰木目」，
     它就這麼說——一個鍵只問一個問題。）

  ⇒ 按 `t/`：找哪些字的拆分裏用了光標下這個字。`t?` 是同一個問題，
     答案畫在另一個工作區。

表頭上面那一行數字是**欄號**：`t20-20g` 去第 20 行第 20 欄，
`t1s` 照第一欄排，`3gd` 改成在第 3 欄裏找。
`:table off` 退出。
